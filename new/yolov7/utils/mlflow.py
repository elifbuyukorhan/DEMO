...
ROOT = FILE.parents[3]
DATA_COMMIT_ID = "f32e144f26d"
DATA_PATH = "HIT-UAV_CAMEL_MONET_runway_combined.dvc"
DATA_REPO = "http://cmbitbucket.stm.com.tr/scm/~muzeyyennur.yilgin/dvc_data_demo.git"


def get_data_from_dvc(data_repo,data_path,data_commit_id):
    try:
        subprocess.run(["git", "clone", data_repo, "--branch", data_commit_id, "--single-branch"])
        subprocess.run(["dvc", "pull", "-R", data_commit_id, "-d", data_path])
    except subprocess.CalledProcessError as e:
        print(f"Error: {e}")
    finally:
        subprocess.run(["rm", "-rf", data_commit_id])
...
def initiate_mlflow_logging(info_dict,hyperparameters,run_name):
    """
    ...
    global mlflow, _mlflow, _run_id, _expr_name, _run_name
    _run_name = run_name
    ...
    _mlflow.log_params(hyperparameters)
    _mlflow.log_param("data_dvc_name", DATA_PATH)
    _mlflow.log_param("data_dvc_commit_id", DATA_COMMIT_ID)
...
def save_last_model(last_ckpt_path):
    ...
    """
    _mlflow.log_artifact(last_ckpt_path)
...
def save_best_model(best_ckpt_path):
    ...
    """
    _mlflow.log_artifact(best_ckpt_path)
    model_uri = f'runs:/{_run_id}/'
    _mlflow.register_model(model_uri, _run_name)